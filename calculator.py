	...
	def __init__(self):
		...
		for r in range(1, 5):
			for col in range(3):
				...
				self.digit_buttons[self.digit-1].grid(row=r, column=col, stick='wens', padx=5, pady=5)
				...
				if self.digit_buttons[self.digit-1]['text'] == '10':
					...
					self.digit_buttons[self.digit-1].grid(row=4, column=1, stick='wens')
					break
				self.digit+=1
		...
		for d in range(len(self.d_buttons)):
				self.button = tk.Button(self.root, text=f'{self.d_buttons[d]}', font=('Arial', 15), command=lambda x=d: self.operator_handler(x))
				self.button.grid(row=d, column=3, stick='wens', padx=5, pady=5)
